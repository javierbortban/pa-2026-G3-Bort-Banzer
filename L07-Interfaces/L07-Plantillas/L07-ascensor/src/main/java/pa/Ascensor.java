...
public class Ascensor {
	...
	private final IAccesoDatos acceso;
	
	//declaración de atributos. Son todos privados.
	
	
	public Ascensor(IAccesoDatos acceso) {
		this.acceso=acceso;
	...
	public int leer_peticiones() {
		...
		int[] pisos=acceso.obtenerDatos();
		for(int destino:pisos) {
	...
	public int analizar_peticicones () {
		...
		while (!peticiones.isEmpty()) { 
			...
			final boolean destino_actual=(destino != piso_actual);
			final boolean validacion=destino_ok && destino_actual;
			if (validacion && (peticiones_validas.size() < max_personas)) {
	...
	public void llevar_personas_a_sus_destinos() {
		...
		while (!peticiones_validas.isEmpty()) {
			...
			int destino=p.getPiso_destino();
			if(piso_anterior!=null && destino==piso_anterior) {
